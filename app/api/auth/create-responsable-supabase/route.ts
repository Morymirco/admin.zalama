...
export async function POST(request: NextRequest) {
  try {
    console.log('🚀 Début création compte Responsable Supabase');
    
    const { email, displayName, partenaireId, phoneNumber, poste, partenaireNom } = await request.json();
    ...
    console.log('📋 Données reçues:', { email, displayName, partenaireId, partenaireNom, poste });
    ...
    console.log('🎉 Création compte Responsable terminée avec succès');
    ...
    console.error('❌ Erreur création Responsable Supabase:', error);
    return NextResponse.json({ 
      error: error.message || 'Erreur création Responsable Supabase',
