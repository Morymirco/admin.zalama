...
interface StatistiquesAlertesProps {
  statistiques: StatistiqueAlerte[];
  isLoading: boolean;
}

const StatistiquesAlertes: React.FC<StatistiquesAlertesProps> = ({ statistiques, isLoading }) => {
  return (
    <div className="mb-8">
      <h2 className="text-xl font-semibold mb-4 text-[var(--zalama-text)]">Activité par type d&apos;alerte</h2>
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <div className="flex items-center mt-1">
                        ...
                        <span className={`ml-2 text-xs ${stat.tendance === 'hausse' ? 'text-[var(--zalama-danger)]' : stat.tendance === 'baisse' ? 'text-[var(--zalama-success)]' : 'text-[var(--zalama-text-secondary)]'}`}>
                ...
                <div className="flex flex-col space-y-2">
                  <div className="flex flex-col">
                    <div className="flex justify-between mb-1">
                      <span className="text-xs text-[var(--zalama-text-secondary)]">Résolues</span>
                      <span className="text-xs font-medium text-[var(--zalama-success)]">{stat.resolues}</span>
                    </div>
                    <div className="w-full bg-[var(--zalama-bg-lighter)] rounded-full h-1.5">
                      <div 
                        className="bg-[var(--zalama-success)] h-1.5 rounded-full" 
                        style={{ width: `${(stat.resolues / stat.nombre) * 100}%` }}
                      ></div>
                    </div>
                  </div>
...
export default StatistiquesAlertes;
