-- ============================================================================
-- CORRECTIF COMPLET : Logique de transaction et remboursement ZaLaMa
-- ============================================================================
...
-- 6. ANALYSE du problème pour comprendre la root cause
SELECT 
...
-- 7. RECOMMANDATIONS pour éviter les futurs problèmes
/*
✅ LOGIQUE CORRECTE DE L'APPLICATION ZaLaMa :

1. UNE SEULE TRANSACTION par demande d'avance
   - Vérifier existingTransactions dans lengo-cashin/route.ts ✅
   - Réutiliser les transactions annulées au lieu de créer de nouvelles ✅

2. UN SEUL REMBOURSEMENT par transaction
   - Le trigger vérifie maintenant NOT EXISTS avant de créer ✅
   - Contrainte unique transaction_id maintenue ✅

3. LOGIQUE FINANCIÈRE ZaLaMa :
   - Employé reçoit : montantDemandé - frais(6.5%)  Ex: 1870 GNF
   - Transaction stocke : montantDemandé             Ex: 2000 GNF
   - Partenaire rembourse : montantDemandé           Ex: 2000 GNF
   - ZaLaMa garde : frais(6.5%)                     Ex: 130 GNF

4. GESTION DES STATUTS :
   - LengoPay SUCCESS → Transaction EFFECTUEE → Trigger remboursement
   - Prioriser lengo_status comme source de vérité ✅
   - Notifications envoyées seulement si EFFECTUEE ✅

PROBLÈMES RÉSOLUS :
- ❌ Doublons de remboursements → ✅ Trigger avec NOT EXISTS
- ❌ Transaction SUCCESS reste ANNULEE → ✅ Mise à jour forcée
- ❌ Contraintes de montant incorrectes → ✅ Contraintes corrigées
- ❌ Notifications échouent → ✅ Statuts cohérents
*/ 